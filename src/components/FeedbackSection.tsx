...
export default function FeedbackSection() {
  ...
  // Google Authentication State & Modal
  ...
  // Google Modal Form State for custom account selection / input
  ...
  const handleConfirmGoogleIdentity = (name: string, email: string) => {
    const finalName = name.trim() || 'Verified Client';
                ...
                <div className="flex items-center gap-2">
                  {!isAuth ? (
                    <button
                      ...
                      type="button"
                      onClick={handleOpenGoogleScreen}
            ...
            <div className={`p-4 rounded-xl border transition-all ${isCodeValid ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-[#121929] border-white/10'}`}>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <KeyRound className="w-4 h-4 text-[#00d9ff]" /> One-Time Feedback Code
                </span>
                {isCodeValid && (
                  <span className="text-emerald-400 text-[11px] font-bold flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Verified Code
                  </span>
                )}
              </label>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      if (!isCodeValid && aliCode.trim().length > 0) {
                        handleVerifyCode();
                      }
                    }
                  }}
                  disabled={isCodeValid}
                  placeholder="Ali-XXXXXX (e.g. Ali-ABC123)"
                ...
                {!isCodeValid ? (
                  <button
                    ...
                    disabled={verifyingCode || !aliCode.trim()}
                  ...
                  <div className="px-4 py-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-xl flex items-center gap-2 text-xs font-bold shrink-0">
                    <CheckCircle2 className="w-4 h-4" /> Validated
                  </div>
                )}
              </div>
              <p className="text-[10px] text-slate-400 mt-2">
                Enter the unique code provided by Muhammad Ali for your project review.
              </p>
            </div>
